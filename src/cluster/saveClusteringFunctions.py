...
def do_dbscan_for_bb_and_sc(combsoutputdir, ifg, vdmresidues, print_clus):
    ...
    for typ in BBorSC:
        ...
        if print_clus==False:
            parsed_clusters, centroids = dbcluster(ifgcoords, pdbs, rmsd=0.75, min_core_num=1,BBorSC=typ,print_clusters=False,outputdir=None,vdmnames=None)
        elif print_clus==True:
            parsed_clusters, centroids = dbcluster(ifgcoords, pdbs, rmsd=0.75, min_core_num=1,BBorSC=typ,print_clusters=True, outputdir=outdir, vdmnames=vdmnames)
...
def dbcluster(iFGcoords_array,pdbs,BBorSC,print_clusters,vdmnames,outputdir,rmsd,min_core_num):
    ...
    for csize,i in sorted(zip(clustersize_db,range(0,n_clusters)),reverse=True):
        for each in indices[labels==i]:
            parsed_clusters[j].append(pdbs[each])
        j+=1
    pair_dist = scipy.spatial.distance.pdist(iFGcoords_array)
    pair_dist_matrix = scipy.spatial.distance.squareform(pair_dist)
    # pair_dist_matrix is the rmsd matrix
        ...
        for i, members in enumerate(parsed_clusters):
            ...
            for r in members:
                ...
                if r[0] in [x[0] for x in cluster_centroids]:
                    ...
                    head = clus_num
                if BBorSC=='SC':
                    outfile = outputdir+'%s_cluster%s'%(vdmnames,head+name)
                elif BBorSC=='N_CA' or BBorSC=='C_O':
                    outfile = outputdir+'cluster%s'%(head+name)
                shutil.copy(orig, outfile)
