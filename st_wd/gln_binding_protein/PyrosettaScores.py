...
def pose_scoring(pose, display_residues = []):
    ...
    scoresdict = {} # key is resnum, value is list 
    hbonddict= {}
    for i in display_residues:
        ...
        for w in [13,14,18]: # these are just omega, dun, rama scores
            scoresdict[i][rosetta.core.scoring.name_from_score_type(weights[w])] = \
                np.round(residue_weighted_energies_matrix[w][i-1],2)
        hbonddict[i] = hbond_dictionary[i][:-1].split('\n')
    return scoresdict, hbonddict

################################################################################
def pyrosetta_scores(design_pdb, residues, score_df):
