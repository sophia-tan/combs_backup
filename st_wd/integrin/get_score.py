...
def get_ifg(int_res, int_res_atoms, targetresn, target_res_atoms):
    ...
    (target = mutated res)
    if the interacting atoms are C or O, take the int_res's C_O ifg and the target res is vdm
    if the target residue's atoms are C/O, take target res's C_O ifg and int res is vdm
    (except if A753 and A785)
    if interacting res has ifg, take it
    else, take target residue's ifg'''
